...
If Linear is a copperplate engraving, Wave is Art Nouveau: organic, rhythmic, and a little hypnotic.
...
- **Art Nouveau illustration:** Low wave height, high curviness, tight interval.
...
- **Op Art and moiré effects:** Stack several Wave fills with different phase shifts to produce interference patterns.
...
Click **+** in the Layers Panel. Select **Wave**. Vexy Lines redraws the image in flowing curves at once.
...
Turn on Dynamic Color for both fills. Set segment length to 5 mm. The wave strokes now pick up greens from foliage, blues from sky, and earth tones from the terrain. The landscape gains colour while keeping the flowing wave texture.
...
Use Wave Height 12, Wave Length 20, Interval 1.5 mm for all three. The offset phases create shimmering interference patterns. Where the waves align, density increases. Where they diverge, gaps open. The result is an optical vibration modulated by your source image.

### Turbulent Water

High wave height (25--40), short wavelength (8--12), curviness at 60% (not fully smooth, because slightly angular waves feel more like choppy water). Randomization at 15%. Angle at 0°. Apply to a water photograph and the wave strokes echo the actual wave patterns beneath.

### Zigzag Energy

Curviness at 0% transforms sine curves into angular zigzags. Combined with a high wave height (20) and short wavelength (10), this produces aggressive, electric energy. It works for music posters, sports graphics, and anything that needs to shout rather than whisper. Add Randomization at 5--10% to prevent the zigzags from looking too computer-generated.

### Vertical Cascade

Angle at 90°. Wave Height at 12. Everything flows downward like rain or falling fabric. It suits portraits where the subject is looking up: the downward wave direction plays against the upward gaze.
...
- **Phase Shift makes the moiré.** Two identical Wave fills with different Phase Shift values create interference patterns that no single fill can produce. Start with a 50% shift difference and adjust from there.
- **Wave + Linear = the best of both.** Use a Wave fill for the main image texture and a Linear fill (different angle) for crosshatch in the shadows. The contrast between flowing and straight lines gives the image depth.
