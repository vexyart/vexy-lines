...
Drop a portrait photo into Vexy Lines. Sixty seconds later, you're looking at a copper-plate engraving that could have taken a master printmaker three weeks to cut by hand. Change two sliders, and it becomes a halftone poster. Change the fill type, and it's a wireframe sculpture. The source photo never leaves your machine, never appears in the output. It is only the map that tells the algorithms where to put ink.

Vexy Lines is a desktop application for macOS and Windows that turns raster images (photographs, illustrations, scans, anything made of pixels) into scalable vector artwork. Instead of tracing edges or converting pixels to vectors one-for-one, it *interprets* image tone through algorithmic fills that generate entirely new vector patterns.
...
This differs from what auto-trace tools do. Adobe Illustrator's Image Trace, for instance, tries to find the edges in your image and draw vector shapes that match them. It's conversion: pixel shapes in, vector shapes out, aiming for fidelity. Vexy Lines does not try to reproduce your photo. It *reinterprets* the photo through a different visual language.
...
**Shape-based fills** work with discrete elements rather than continuous lines. **Halftone** places dots, squares, or other shapes on a grid, sizing each by the brightness of the pixel beneath it. This is the same principle behind newspaper printing, now under your control. **Trace** detects edges in the source image and draws contour lines along them, producing a topographic-map effect.
...
Each fill has its own set of parameters, but they share a common core: **Interval** (how far apart the strokes sit), **Angle** (the rotation of the pattern), and **Image Threshold** (which brightness values produce strokes at all). These three controls alone give you a wide range.

## What You Can Make

The range of output is broad:

- **Fine art prints.** Stack a Linear fill at 0 degrees with another at 90 degrees and you have crosshatching in the manner of hand-engraved work.
...
- **Generative textures.** Fractal and Scribble fills generate patterns that work well as background textures, fabric prints, or packaging elements.
...
A single fill applied to an entire image is a start, but the range comes from composition.
...
Vexy Lines began life in 2007 as **Strokes Maker**, created by Dmitry Apanovich. Strokes Maker earned a dedicated following among illustrators, printmakers, and currency designers: people who understood the craft of engraving and saw that software could replicate its visual language while removing weeks of manual labour.
...
A **free demo** lets you use every feature except export, so you can see exactly what Vexy Lines produces before spending anything. When you're ready, a **lifetime license costs $99**, covers **two computers**, and works offline after activation. No subscriptions, no recurring charges, no cloud dependency.
