...
There's no GPU requirement. All rendering is CPU-based, so a faster processor with more cores will improve refresh times on complex documents, but a five-year-old laptop still handles typical projects.
...
On first launch, macOS may warn you that the application was "downloaded from the internet." Click **Open** to confirm. This is standard macOS Gatekeeper behaviour for applications downloaded outside the App Store. Vexy Lines is signed and notarized.
...
2. Follow the installation wizard. The default location is `C:\Program Files\Vexy Lines\`. There is rarely a reason to change it.
3. Choose whether to create a desktop shortcut and Start Menu entry.
4. Click **Finish**.

Windows may show a SmartScreen prompt on first run. Click **More info**, then **Run anyway**. This happens because Vexy Lines is a niche application without millions of downloads, and SmartScreen uses popularity as a trust signal.
...
Your license covers **two computers**. You can use Vexy Lines on your desktop and your laptop, or on a Mac and a Windows machine, in any combination. If you need to move your license to a new computer, deactivate it first on the old one (Help > Deactivate License), then activate on the new machine.
...
You can build a complete, complex document in demo mode and see exactly what the output would look like. When you're ready to export, enter your license key and your existing work is immediately exportable.
...
They take about sixty seconds and orient you in the interface. If you dismiss them early or want to see them again later, choose **Help > Intro Tour** from the menu bar to replay the sequence.

## Checking for Updates

Vexy Lines checks for updates automatically on launch (it needs an internet connection for this). When an update is available, you'll see a notification with the option to download and install it.
...
2. Click **Buy** and complete the purchase. The price is **$99 for a lifetime license**, with no subscription and no renewal fees.
