...
Vexy Lines shifts each line perpendicular to its direction. At 0%, all lines are evenly spaced. At 50%, each line is offset from its ideal position by up to half the interval distance, in either direction. Lines can bunch together or spread apart, but they remain parallel and straight (or sinusoidal, for Wave).
...
### Trace
Contour lines are displaced from their calculated positions. Light randomisation makes contour maps look hand-drawn. Heavy randomisation makes them look like they were traced during an earthquake.
...
- **Randomisation** moves each element independently by a different random amount. The pattern structure is disrupted. Randomisation is stochastic: refreshing the fill produces a slightly different result each time (though Vexy Lines stores the random seed, so a saved file always looks the same when reopened).

Use Shift to align a pattern with a specific feature in the image, such as centering a halftone grid on a face. Use Randomisation to break the mechanical feel of the pattern.
...
For most illustration work, a small amount of randomisation improves the result. The viewer's eye is extremely sensitive to perfect regularity: it reads as "computer-generated" even in high-quality work. A 5–10% randomisation value is the equivalent of a guitarist's natural timing variation: technically imperfect, perceptually better.
...
1. Start at 0% and evaluate the fill's mechanical appearance. Ask whether this perfection serves the image.
2. Increase to 5%. The change is subtle. Look for it in the evenness of stroke spacing: gaps vary slightly and the pattern feels less rigid.
