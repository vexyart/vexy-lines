...
You have a photograph on the canvas: a face, a landscape, a still life, anything with tonal variety. You're about to turn it into vector art with a single click and two slider adjustments.
...
**Interval is the parameter that changes the picture most in Vexy Lines.** If you never touch another control, this one alone gives you a range from photographic fineness to bold graphic abstraction. Here's a rough guide to what different values produce:
...
Set it wherever you like. There's no "correct" Interval. It depends on the size of your document, the detail in your source image, and the look you want. For this first experiment, somewhere around **1.0 mm** gives a satisfying balance of detail and clarity.
...
- **45 degrees**: The classic engraving diagonal. Most traditional engravings use a diagonal because it avoids aligning with the horizontal and vertical edges of the image, which can create visual interference patterns. This is the "safe" choice, and it usually looks good.
...
Try rotating the angle slowly and watch how the character of the image changes. A portrait that looks stoic with vertical lines looks dynamic with diagonal lines and gentle with horizontal ones. The same data, the same algorithm, the same spacing, only a rotation, and the emotional register shifts.
...
This transforms the output from monochrome engraving to full-colour vector illustration. It's one checkbox.

Dynamic Color works with any fill type, not just Linear. But it's most striking on your first Linear fill because the parallel lines create a recognisable colour image, like a colour photograph rendered entirely in strokes.
...
And you have a piece of vector artwork in the manner of a hand-crafted engraving. Those strokes are real vector paths you can open in Illustrator, edit point by point, scale to any size, and print at any resolution.

This is the core of Vexy Lines. Everything else in the application (the eleven other fill types, the masking system, the layers and groups, the mesh warping, the threshold histogram) builds on this foundation. Those features give you control, subtlety, and creative range. But the fundamental transaction is always the same: brightness values go in, vector strokes come out.
...
Now you have two sets of parallel lines crossing each other: classic **crosshatching**. In the dark areas of the image, both sets of lines are thick, creating a dense mesh. In the bright areas, both sets thin out or disappear. In the mid-tones, the intersection of two stroke directions creates a woven texture in the manner of traditional printmaking.

This is the "one fill is a sketch, two fills become an engraving" principle. Each additional fill layer adds depth and visual complexity. Three fills at different angles approach the look of a traditional engraving. Four fills with varying intervals start to feel like mezzotint.
...
3. **Modulated each line's thickness** based on the luminance of the source image pixels along that line's path. Dark pixels give thick strokes. Bright pixels give thin strokes (or gaps).
...
- [Tuning Your Artwork](107-adjusting-fills.md) covers every parameter in the Properties Panel, including the Image Threshold histogram and the Stroke Thickness modes.
...
Or, if you can't resist experimenting, click that **+** button again and try a fill type you haven't used yet. Wave, Halftone, and Scribble each give a visible result at once.
