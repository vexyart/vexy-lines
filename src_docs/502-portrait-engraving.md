...
3. **Crop tight.** Head and shoulders is the classic engraving frame. If the subject is small in the photo, crop before importing: more pixels for the face means more detail in the fill.

4. **Remove distracting elements.** A logo on a t-shirt, a stray hair across the forehead, a bright earring: each of these generates strokes. Clean them up or plan to mask around them.
...
File > New. Drag your portrait onto the canvas. The photo becomes the source image: the brightness map that every fill reads.
...
Set Image Threshold to highlights only: drag the left (shadow) handle to 170. The Circular fill now only generates rings where the source image is bright. If the background is lighter than the face, the rings appear behind the head as a gentle radial glow.
...
- **Threshold: Full range (0–255).** This layer handles everything. It is the backbone of the portrait.
...
Mask to the hair only. Auto-detect the hair area or paint it with the Brush. The Scribble fill's chaotic strokes contrast with the disciplined lines on the face, so skin and hair read as different textures.
...
| Single-line (no crosshatch) | Remove Layer 3 (crosshatch) and Layer 5 (deep shadow). Relies on Layer 2 alone. Cleaner, simpler, less tonal range. |
