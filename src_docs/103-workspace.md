...
Every tool, panel, and control in Vexy Lines is visible from the moment you open the application. There are no hidden drawers, no floating palettes to hunt for, no modes that rearrange the screen. Learn this layout once and you know where everything is.
...
- **Masks** appear attached to their parent layer or group, as a small thumbnail showing white (visible) and black (hidden) regions.
...
The canvas is interactive as well as a display:
