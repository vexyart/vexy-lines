...
**Tips:**
- Match Interval to your intended viewing distance. Fine intervals read well up close but merge into flat grey from across a room. Larger intervals read clearly from a distance.
...
**Why it matters:** Angle changes the visual character without changing the density or tone. Horizontal lines feel calm. Diagonal lines feel dynamic. Vertical lines feel solemn. Same image, same interval: a 45-degree rotation changes the mood.
...
- Shift is also useful for precisely positioning a pattern relative to a specific feature in the source image, such as centring a ring pattern over an eye.
...
Each fill layer in Vexy Lines is independent: its own algorithm, its own parameters, its own mask. Stacking fills is how you build complex, layered artwork:
...
To control *where* each fill appears (the Halftone fill only on the background, the Linear fill only on the face), you use **masks**. That's the next article: [Hiding and Revealing](108-basic-masks.md).
