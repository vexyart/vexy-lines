...
Static and dynamic are per-fill settings, not a document-wide choice. This means you can stack multiple fills and use different colour modes for each:
...
Both static and dynamic colour fills support an **Opacity** slider (0–100%). At 100%, strokes are fully opaque and cover whatever is beneath them. At lower values, underlying fills and the background show through.
