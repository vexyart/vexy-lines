...
When a pixel falls outside the threshold range, Vexy Lines does not generate a stroke at that position. The stroke does not become invisible or zero-width; it simply does not exist. This is different from setting Thickness Min to zero, which creates invisible geometry that still bloats your file. Threshold exclusion is clean: no pixel, no stroke, no data.
...
This is one of the most useful techniques in Vexy Lines, and it relies entirely on Image Threshold.
